// Optimized Dashboard with Performance Improvements
...
export default function OptimizedDashboardPage() {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      // Check for errors
      if (bikesResult.error || appointmentsResult.error || remindersResult.error) {
        setError('Failed to load some dashboard data. Please refresh the page.');
      }

      setBikes(bikesResult.bikes || []);
  ...
  // Paginated data
  const displayedBikes = showAllBikes ? bikes : bikes.slice(0, BIKES_PER_PAGE);
  const displayedAppointments = showAllAppointments
    ? appointments
    : appointments.slice(0, APPOINTMENTS_PER_PAGE);
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-indigo-900 via-purple-800 to-pink-700">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              <button
                ...
                className="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg transition-all font-semibold"
              >
                Find Service
              </button>
              <button
      ...
      {/* OPTIMIZATION: Show skeleton while loading instead of blank screen */}
      {loading ? (
        <DashboardSkeleton />
      ) : error ? (
                            ...
                            <div className="flex-1">
                              ...
                              {bike.currentMileage && (
                                <p className="text-sm text-gray-600">
                                  Mileage: {bike.currentMileage.toLocaleString()} miles
                                </p>
                              )}
                    ...
                    {bikes.length > BIKES_PER_PAGE && (
                      <div className="mt-4 text-center">
                        ...
                        >
                          {showAllBikes
                            ? 'Show Less'
                            : `Show All (${bikes.length - BIKES_PER_PAGE} more)`}
                        </button>
                      </div>
                    )}
                  </>
              ...
              <div className="p-6">
                {appointments.length === 0 ? (
                  <div className="text-center py-12 text-gray-500">
                    ...
                    <p className="text-sm">Request service to find nearby shops</p>
                  </div>
                ) : (
                  <>
                    <div className="space-y-4">
                    ...
                    {appointments.length > APPOINTMENTS_PER_PAGE && (
                      <div className="mt-4 text-center">
                        ...
                        >
                          {showAllAppointments
                            ? 'Show Less'
                            : `Show All (${appointments.length - APPOINTMENTS_PER_PAGE} more)`}
                        </button>
                      </div>
                    )}
                  </>
              ...
              <div className="p-6">
                <div className="space-y-4">
                  {reminders.slice(0, 5).map((reminder) => (
                    <div
                      key={reminder.id}
                      className="border-l-4 border-pink-500 bg-pink-50 p-4 rounded-r-lg"
                    >
                      <div className="flex items-start justify-between">
                        <div>
